// Generated by Flexc++ V2.14.00 on Mon, 05 Feb 2024 10:29:28 +0000

#ifndef LexBASE_H_INCLUDED
#define LexBASE_H_INCLUDED
...
private:
    ...
    size_t (LexBase::*d_get)() = &LexBase::getInput;
...
protected:
    ...
    int     const (*d_dfaBase_)[62];

    static int     const s_dfa_[][62];
    static int     const (*s_dfaBase_[])[62];
...
public:
    LexBase(LexBase const &other)             = delete;
...
protected:
    LexBase(std::istream &in, std::ostream &out, bool keepCwd = true);
    LexBase(std::string const &infilename, std::string const &outfilename, bool keepCwd = true);
    ~LexBase();
...
inline size_t LexBase::Input::lineNr() const
{
    return d_lineNr;
}
inline size_t LexBase::Input::nPending() const
{
    return d_deque.size();
}
inline void LexBase::Input::setPending(size_t size)
{
    d_deque.erase(d_deque.begin(), d_deque.end() - size);
}
inline void LexBase::Input::close()
{
    delete d_in;
...
inline LexBase::~LexBase()
{
    d_input->close();
}

template <typename ReturnType, typename ArgType>
inline ReturnType constexpr LexBase::as(ArgType value)
{
    return static_cast<ReturnType>(value);
}

// $insert startcondimpl
inline LexBase::StartCondition_ constexpr LexBase::SC(int sc)
{
    return as<StartCondition_>(sc);
}

inline int constexpr LexBase::SC(StartCondition_ sc)
{
    return as<int>(sc);
}

inline LexBase::StartCondition_ LexBase::startCondition() const
{
    return SC(d_startCondition);
}

inline void LexBase::begin(StartCondition_ startCondition)
{
    // d_state is reset to 0 by reset_()
    d_dfaBase_ = s_dfaBase_[d_startCondition = SC(startCondition)];
}

inline bool LexBase::knownFinalState()
{
    return (d_atBOL && available(d_final.bol.rule)) ||
           available(d_final.std.rule);
}

inline bool constexpr LexBase::available(size_t value)
{   
    return value != std::numeric_limits<size_t>::max();
}

inline std::ostream &LexBase::out()
{
    return *d_out;
}

inline void LexBase::push(size_t ch)
{
    d_input->reRead(ch);
}

inline void LexBase::push(std::string const &str)
{
    d_input->reRead(str, 0);
}


inline std::vector<LexBase::StreamStruct> const &LexBase::streamStack() const
{
...
inline void LexBase::setFilename(std::string const &name)
{
    d_filename = name;
}

inline void LexBase::setMatched(std::string const &text)
{
    d_matched = text;
}

inline std::string const &LexBase::matched() const
{
    return d_matched;
}

inline std::string const &LexBase::cwd() const
{
    return d_cwd;
}

inline std::string const &LexBase::filename() const
{
    return d_filename;
}

inline void LexBase::echo() const
{
    *d_out << d_matched;
}

inline size_t LexBase::length() const
{
    return d_matched.size();
}

inline void LexBase::leave(int retValue) const
{
    throw as<Leave_>(retValue);
}

inline size_t LexBase::lineNr() const
{
    return d_input->lineNr();
}

inline void LexBase::more()
{
    d_more = true;
}

inline size_t LexBase::state_() const
{
    return d_state;
}

inline size_t LexBase::get_()
{
    return (this->*d_get)();
}

inline size_t LexBase::getInput()
{
    return d_input->get();
}

inline bool LexBase::return_()
{
    return d_return;
}

inline void LexBase::noReturn_()
{
    d_return = false;
}


#endif //  LexBASE_H_INCLUDED
