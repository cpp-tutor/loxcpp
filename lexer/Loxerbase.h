...
private:
    ...
    size_t (LoxerBase::*d_get)() = &LoxerBase::getInput;
...
protected:
    ...
    int     const (*d_dfaBase_)[59];

    static int     const s_dfa_[][59];
    static int     const (*s_dfaBase_[])[59];
...
public:
    LoxerBase(LoxerBase const &other)             = delete;
...
protected:
    LoxerBase(std::istream &in, std::ostream &out, bool keepCwd = true);
    LoxerBase(std::string const &infilename, std::string const &outfilename, bool keepCwd = true);
    ~LoxerBase();
...
inline size_t LoxerBase::Input::lineNr() const
{
    return d_lineNr;
}
inline size_t LoxerBase::Input::nPending() const
{
    return d_deque.size();
}
inline void LoxerBase::Input::setPending(size_t size)
{
    d_deque.erase(d_deque.begin(), d_deque.end() - size);
}
inline void LoxerBase::Input::close()
{
    delete d_in;
...
inline LoxerBase::~LoxerBase()
{
    d_input->close();
}

template <typename ReturnType, typename ArgType>
inline ReturnType constexpr LoxerBase::as(ArgType value)
{
    return static_cast<ReturnType>(value);
}

// $insert startcondimpl
inline LoxerBase::StartCondition_ constexpr LoxerBase::SC(int sc)
{
    return as<StartCondition_>(sc);
}

inline int constexpr LoxerBase::SC(StartCondition_ sc)
{
    return as<int>(sc);
}

inline LoxerBase::StartCondition_ LoxerBase::startCondition() const
{
    return SC(d_startCondition);
}

inline void LoxerBase::begin(StartCondition_ startCondition)
{
    // d_state is reset to 0 by reset_()
    d_dfaBase_ = s_dfaBase_[d_startCondition = SC(startCondition)];
}

inline bool LoxerBase::knownFinalState()
{
    return (d_atBOL && available(d_final.bol.rule)) ||
           available(d_final.std.rule);
}

inline bool constexpr LoxerBase::available(size_t value)
{   
    return value != std::numeric_limits<size_t>::max();
}

inline std::ostream &LoxerBase::out()
{
    return *d_out;
}

inline void LoxerBase::push(size_t ch)
{
    d_input->reRead(ch);
}

inline void LoxerBase::push(std::string const &str)
{
    d_input->reRead(str, 0);
}


inline std::vector<LoxerBase::StreamStruct> const &LoxerBase::streamStack() const
{
...
inline void LoxerBase::setFilename(std::string const &name)
{
    d_filename = name;
}

inline void LoxerBase::setMatched(std::string const &text)
{
    d_matched = text;
}

inline std::string const &LoxerBase::matched() const
{
    return d_matched;
}

inline std::string const &LoxerBase::cwd() const
{
    return d_cwd;
}

inline std::string const &LoxerBase::filename() const
{
    return d_filename;
}

inline void LoxerBase::echo() const
{
    *d_out << d_matched;
}

inline size_t LoxerBase::length() const
{
    return d_matched.size();
}

inline void LoxerBase::leave(int retValue) const
{
    throw as<Leave_>(retValue);
}

inline size_t LoxerBase::lineNr() const
{
    return d_input->lineNr();
}

inline void LoxerBase::more()
{
    d_more = true;
}

inline size_t LoxerBase::state_() const
{
    return d_state;
}

inline size_t LoxerBase::get_()
{
    return (this->*d_get)();
}

inline size_t LoxerBase::getInput()
{
    return d_input->get();
}

inline bool LoxerBase::return_()
{
    return d_return;
}

inline void LoxerBase::noReturn_()
{
    d_return = false;
}


#endif //  LoxerBASE_H_INCLUDED
